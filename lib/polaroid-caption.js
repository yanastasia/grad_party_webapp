...
async function readFont(packageName, filename) {
  const packageRoot = path.dirname(require.resolve(`${packageName}/400.css`));
  const fileBuffer = await fs.readFile(path.join(packageRoot, 'files', filename));
...
async function loadFonts() {
  if (!fontsPromise) {
    fontsPromise = Promise.all([
      readFont('@fontsource/allura', 'allura-latin-400-normal.woff'),
      readFont('@fontsource/cormorant-garamond', 'cormorant-garamond-latin-400-normal.woff'),
      readFont('@fontsource/cormorant-garamond', 'cormorant-garamond-cyrillic-400-normal.woff')
        .catch(() => readFont('@fontsource/cormorant-garamond', 'cormorant-garamond-cyrillic-ext-400-normal.woff')),
...
function captionPlan(caption) {
  ...
  if (useScript) {
    ...
    return { family: 'allura', initialSize: 72, minSize: 48, lineHeight: 1.20, maxLines: 4 };
  }

  if (length <= 120) return { family: 'cormorant', initialSize: 72, minSize: 46, lineHeight: 1.16, maxLines: 4 };
  if (length <= 160) return { family: 'cormorant', initialSize: 60, minSize: 42, lineHeight: 1.15, maxLines: 5 };
  return { family: 'cormorant', initialSize: 52, minSize: 36, lineHeight: 1.14, maxLines: 5 };
}
...
function fitCaption(caption, plan, fonts) {
  for (let fontSize = plan.initialSize; fontSize >= plan.minSize; fontSize -= 2) {
  ...
  const fontSize = plan.minSize;
  let lines = wrapMeasured(caption, fontSize, plan, fonts);
  if (lines.length > plan.maxLines) {
    lines = lines.slice(0, plan.maxLines);
    const last = lines[lines.length - 1];
    const lastText = `${last.tokens.join(' ')}…`;
    last.tokens = [lastText];
    last.width = tokenWidth(lastText, fontSize, plan, fonts);
  }
  return { fontSize, lines, lineHeightPx: fontSize * plan.lineHeight };
}
...
function captionPathsSvg(caption, layout, plan, fonts) {
...
export async function createCaptionedPolaroid(processedBuffer, caption) {
  ...
  return sharp({
    ...
    .composite([
      ...
      { input: captionPathsSvg(cleanCaption, layout, plan, fonts), blend: 'over' },
    ])
    .jpeg({ quality: 94, chromaSubsampling: '4:4:4' })
    .toBuffer();
}
